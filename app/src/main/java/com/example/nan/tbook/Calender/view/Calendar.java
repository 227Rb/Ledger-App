...
import com.example.nan.tbook.Calender.component.CalendarRenderer;
...
public class Calendar extends View {
     ...
     * 日历列数
     */
    private CalendarAttr.CalendarType calendarType; //存放显示模式的参数,控制显示的列数
    private int cellHeight; // 单元格高度
    private int cellWidth; // 单元格宽度

    private OnSelectDateListener onSelectDateListener;    // 单元格点击回调事件
    private Context context;
    private CalendarAttr calendarAttr;      //自定义属性对象
    private CalendarRenderer renderer;

    private OnAdapterSelectListener onAdapterSelectListener;          //匹配点击状态的接口

    private float touchSlop;                                        //触发移动的最短距离参数

    public Calendar(Context context,                                //构造 带监听,初始化的控件
        ...
        private void init(Context context) {                    //初始化
        this.context = context;
        touchSlop = Utils.getTouchSlop(context);                //获得触发移动的最短距离
        initAttrAndRenderer();
    }
