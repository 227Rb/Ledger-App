...
public class RecyclerViewBehavior extends CoordinatorLayout.Behavior<RecyclerView> {
    ...
    private void initMinOffsetAndInitOffset(CoordinatorLayout parent,
        ...
        if (monthPager.getBottom() > 0 && initOffset == -1) {
            initOffset = MonthPagerBehavior.getcY();
            saveTop(initOffset);
        }
        if (!initiated) {
            initOffset = MonthPagerBehavior.getcY();
        ...
        minOffset = MonthPagerBehavior.getcY()/MonthPagerBehavior.getRowY();
    }
...
//        }
//
//        if(MonthPagerBehavior.getcY()==630){Log.d("ldf5","强制执行"); initOffset=630;} else{initOffset = 525;}//当显示多一行时  调整判断参数
//        // 上滑，正在隐藏顶部的日历
//        hidingTop = dy > 0 && child.getTop() <= initOffset // 此child为RecylerView gettop得到与coordinatorLayout的顶部距离
//                && child.getTop() > getMonthPager(coordinatorLayout).getCellHeight();//等于时滑动结束
//
////        Log.e("ldf5"," "+ (dy > 0)+" "+ (child.getTop() <= initOffset)+" "+(child.getTop() > getMonthPager(coordinatorLayout).getCellHeight()));
////                Log.e("ldf5", "1"+" dy="+dy+" getTop()="+child.getTop()+" initOffset="+initOffset+" getCellHeight()="+getMonthPager(coordinatorLayout).getCellHeight()+" "+hidingTop);
//
//
//
//        // 下滑，正在展示顶部的日历
...
//                Utils.scrollTo(parent, child,MonthPagerBehavior.getcY()/MonthPagerBehavior.getRowY(), 500);
//            } else {
//
//                Utils.scrollTo(parent, child,MonthPagerBehavior.getcY(), 150);
//            }
//        } else {
//
//            if (Utils.loadTop() - minOffset > Utils.getTouchSlop(context) && showingTop) {
//                Utils.scrollTo(parent, child,MonthPagerBehavior.getcY(), 500);
//            } else {
